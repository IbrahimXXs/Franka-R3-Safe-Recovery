...
def reference(bench,case,seed,path,checkpoints,stop_step=None):
    ...
    def command_fields():
        return dict(command_offset_x_mm=command['offset_x_mm'],command_offset_y_mm=command['offset_y_mm'],
    ...
    try:
        ...
        for step in range(1,(stop_step or count)+1):
            t=step*bench.dt
            # Causal, monotone depth feedback; a stall cannot ramp tilt by time.
...
def _run(args,app):
    ...
    case_plan=load_plan(args.case_plan) if args.case_plan is not None else None
    ...
    if args.checkpoints is None:args.checkpoints=list(p.checkpoints_mm) if args.mode=='collect' else []
    ...
    if args.resume:
        ...
        for name in manifest['sources']:
            src=Path(__file__).parents[1]/'research'/name if name in ('forge_protocol.py','phase2_protocol.py','forge_collection.py','phase2b.py') else Path(__file__).with_name(name)
            ...
            for cp in cps:
                if cp['reached'] and cp['prefix_numerically_valid']:
                    for policy in ('straight','realign'):
                        tag=f'terminal_{policy}' if cp.get('checkpoint_kind')=='terminal' else f'd{cp["depth_mm"]:g}_{policy}'
