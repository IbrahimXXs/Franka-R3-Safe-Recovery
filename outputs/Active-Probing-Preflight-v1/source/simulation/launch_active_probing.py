"""Launch the bounded active micro-probing pilot with native FORGE settings."""
import argparse
...
parser.add_argument('--output-dir',type=Path,default=root/'outputs/Active-Probing-Pilot-v1')
parser.add_argument('--config',type=Path,default=root/'experiments/forge_phase2.json')
parser.add_argument('--plan',type=Path,default=root/'experiments/active_probing_pilot.json')
...
if not args.config.is_file() or not args.plan.is_file():parser.error('Missing protocol or pilot plan')
...
from active_probing import run
try:
