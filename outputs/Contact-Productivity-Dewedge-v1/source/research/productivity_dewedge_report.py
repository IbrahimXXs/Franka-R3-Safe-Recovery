    ...
    for run in m['runs']:
        ...
        for i,r in enumerate(rows):
            ...
            if r['intervention_started']:
                ...
                d.reset();anchor=r;event=ee[r['intervention_count']-1]
...
def analyze(directory):
    ...
    for j,policy in enumerate(('axial','dewedge')):
        ...
        vv=[e for e in ee if e['online_verified'] or policy=='axial']
        ...
        failed=[e for e in ee if policy=='dewedge' and not e['online_verified']]
        if failed:axes[1].scatter([e['before_mean_normal_load_n'] for e in failed],[e['after_mean_normal_load_n'] for e in failed],facecolors='none',edgecolors=colors[policy],label='de-wedging: unverified endpoint')
