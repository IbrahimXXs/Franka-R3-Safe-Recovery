"""Launch the bounded verified actual-unloading comparison with native FORGE settings."""
import argparse
...
parser.add_argument('--output-dir',type=Path,default=root/'outputs/Contact-Productivity-Dewedge-v1')
parser.add_argument('--config',type=Path,default=root/'experiments/forge_phase2.json')
parser.add_argument('--plan',type=Path,default=root/'experiments/productivity_dewedge_pilot.json')
parser.add_argument('--repeats',type=int,choices=(2,),default=2)
parser.add_argument('--calibration',type=Path,default=root/'experiments/productivity_control_calibration.json')
...
from productivity_dewedge import run
try:
