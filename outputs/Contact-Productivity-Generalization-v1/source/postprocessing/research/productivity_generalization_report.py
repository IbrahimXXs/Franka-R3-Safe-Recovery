...
def analyze(directory):
    ...
    for condition in plan['cases']:
        ...
        for run in runs:
            ...
            summary=dict(**run,safety_stop=not safe(rows[-1],p),recovery_count=len(features),
                ...
                eligible_detector_checks=sum(c['eta_valid'] for c in checks),
                last_eligible_eta_raw=next((c['eta_raw'] for c in reversed(checks) if c['eta_valid']),None),
                minimum_eligible_eta_raw=min((c['eta_raw'] for c in checks if c['eta_valid']),default=None),
                terminal_productivity_consecutive=checks[-1]['productivity_consecutive'] if checks else 0,
                last_check_time_s=checks[-1]['time_s'] if checks else None,terminal_time_s=rows[-1]['time_s'],
                terminal_grasp_slip_mm=rows[-1]['grasp_slip_mm'],terminal_grasp_slip_deg=rows[-1]['grasp_slip_deg'],
                terminal_wrist_force_n=rows[-1]['wrist_force_n'],terminal_wrist_torque_nm=rows[-1]['wrist_torque_nm'],
                terminal_normal_load_n=rows[-1]['normal_load_n'],terminal_min_separation_mm=rows[-1]['min_separation_mm'],
        ...
        for a,b in combinations(POLICIES,2):
            ...
            pairs.append(dict(case_id=condition['case_id'],family=condition['family'],severity=condition['severity'],policy_a=a,policy_b=b,prefix_matched=matched,
                ...
                interventions_a=by[a]['recovery_count'],interventions_b=by[b]['recovery_count'],both_no_intervention=by[a]['recovery_count']==0 and by[b]['recovery_count']==0,
    ...
    for baseline in ('nominal','force','axial'):
        ...
        for scope,rr in (('all_conditions',pp),('strict_prefix_matched',[r for r in pp if r['prefix_matched']])):
            effects.append(dict(baseline=baseline,scope=scope,no_intervention_discordant_pairs=sum(r['both_no_intervention'] and r['success_delta']!=0 for r in rr),no_intervention_wins=sum(r['both_no_intervention'] and r['success_delta']>0 for r in rr),no_intervention_losses=sum(r['both_no_intervention'] and r['success_delta']<0 for r in rr),**paired_effect(rr,plan['analysis']['paired_bootstrap_seed'],plan['analysis']['paired_bootstrap_samples'])))
...
def plots(directory,rows,totals,strata,events,conditions,plt):
    ...
    threshold=json.loads((directory/'experiment.json').read_text())['protocol']['success_depth_mm']
    quiet=[c['case_id'] for c in conditions if all(r['recovery_count']==0 for r in rows if r['case_id']==c['case_id'])]
    fig,ax=plt.subplots(figsize=(14,5))
    for pi,policy in enumerate(POLICIES):
        values=[next(r['final_depth_mm']-threshold for r in rows if r['case_id']==cid and r['policy']==policy) for cid in quiet]
        ax.scatter(np.arange(len(quiet))+(pi-1.5)*.16,values,label=policy,color=COLORS[policy],s=28)
    ax.axhline(0,color='black',ls=':')
    ax.set(xticks=np.arange(len(quiet)),xticklabels=[cid.split('_')[0] for cid in quiet],ylabel='Final depth minus success depth (mm)',xlabel='Condition ID (full parameters in per_condition_results.csv)',title='Conditions with no intervention by any policy: endpoint variability')
    ax.tick_params(axis='x',rotation=90);ax.grid(axis='y',alpha=.2);ax.legend(ncol=4)
    fig.tight_layout();fig.savefig(directory/'no_intervention_margins.png',dpi=150);plt.close(fig)
    fig,axes=plt.subplots(1,3,figsize=(14,4))
...
def report(directory,m,plan,totals,effects,strata,failures,audit):
    ...
    no_action=[r for r in effects if r['scope']=='all_conditions']
    lines+=['','No-intervention discordances (de-wedging wins / losses versus baseline): '+ '; '.join(f"{r['baseline']} {r['no_intervention_wins']} / {r['no_intervention_losses']}" for r in no_action)+'. Neither policy intervened in those pairs, so their outcome differences cannot be attributed to recovery actions. They remain in the primary all-condition results.']
    ...
    for r in df:lines.append(f"| {r['case_id']} | {r['final_offset_x_mm']:+.2f}, {r['final_offset_y_mm']:+.2f}; {r['final_roll_deg']:+.2f}, {r['final_pitch_deg']:+.2f} | {r['ramp_onset_mm']:g} | {r['failure_class']} | {r['final_depth_mm']:.3f} | {r['recovery_count']} | {r['verified_unloadings']} | [history]({r['condition_plot']}) |")
    one_check=[r for r in df if r['safety_stop'] and r['recovery_count']==0 and r['terminal_productivity_consecutive']==1]
    if one_check:
        lines+=['',f"{len(one_check)} de-wedging runs hit a hard safety limit with only one consecutive low-productivity check recorded and no recovery yet initiated. The unchanged two-check detector had not authorized intervention. These are failures before recovery, not evidence that the de-wedging action itself failed. Last eligible eta, check time, consecutive count and terminal safety measurements are retained in failure_cases.csv."]
    terminal_timeouts=[r for r in df if r['failure_class']=='no_policy_trigger_before_timeout' and r['terminal_phase']=='hold']
    if terminal_timeouts:
        lines+=['',f"{len(terminal_timeouts)} de-wedging runs timed out in the final command hold without intervention. The original detector requires recent positive commanded insertion progress, so hold-phase history is ineligible. Failure records include minimum/last eligible eta and final depth: adequate productivity while moving does not guarantee the success-depth criterion is eventually reached. No detector, endpoint or success threshold was changed."]
    ...
    lines+=['',f"Validation passed for {audit['physics_rows']} physics rows and {audit['causal_checks']} causal checks. The audit replays frozen detector decisions, bounded force commands, axial/de-wedging recovery state machines, achieved-motion dwell and retry-pose retention. {audit['source_archives_verified']} source snapshots verified; {audit['preexisting_outputs_checked']} pre-existing output files checked, changed: 0. Exact scene/configuration and calibration match the previous pilot. Frozen plan/policy hashes are checked before and during collection.",
        ...
        '', '![Policy comparison](policy_comparison.png)', '', '![Stratified success](stratified_success.png)', '', '![Every condition](condition_outcomes.png)', '', '![No-intervention endpoint margins](no_intervention_margins.png)', '', '![Recovery](recovery_comparison.png)', '', '![Wrench and load](wrench_load_comparison.png)']
