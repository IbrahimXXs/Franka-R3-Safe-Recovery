...
def event_features(rows,event,next_event,p,hz,target=.5):
    ...
    hand_command=max((r['command_hand_position_2']-anchor['hand_pose_2'])*1000 for r in rr[1:]) if len(rr)>1 else 0.
    # Use the authored peg-depth reference; the float32 hand target has nanometre-scale rounding.
    commanded=max((anchor['depth_mm']-r['command_depth_mm']) for r in rr[1:]) if len(rr)>1 else 0.
    ...
    result=dict(intervention=event['intervention'],trigger_time_s=t0,trigger_depth_mm=anchor['depth_mm'],
        trigger_eta_raw=event['eta_raw'],commanded_retract_mm=max(0.,commanded),hand_target_retract_mm=max(0.,hand_command),actual_retract_mm=float(actual[-1]),
        ...
        rejoin_started=join is not None,rejoin_start_time_s=rows[join]['time_s'] if join is not None else None,
    ...
    for run in m['runs']:
        ...
        for i,r in enumerate(rows):
            ...
            if run['policy']=='verified' and r['phase'] in ('stop','unload_retract','unload_hold'):
                ...
                assert abs(unloader.depth0-r['command_depth_mm']-command)<1e-8
...
def analyze(directory):
    ...
    for j,policy in enumerate(('fixed','verified')):
        ...
        failed=[e for e in ee if policy=='verified' and not e['online_verified']]
        if failed:axes[1].scatter([e['before_mean_normal_load_n'] for e in failed],[e['after_mean_normal_load_n'] for e in failed],facecolors='none',edgecolors=colors[policy],label='verified arm: failed target')
    ...
    partial=[e for e in events if e['policy']=='verified']
    if partial:
        decreased=sum(e['contact_load_decreased'] for e in partial)
        lines+=['',f"Across all {len(partial)} verified-arm recovery attempts, including failures, mean contact load decreased in {decreased}. These are endpoint comparisons after partial or verified motion; they do not establish that the 0.5 mm target was met.",
            '', '| Verified-arm case | Attempts | Verified | Actual retreat at attempt end (mm), range | Mean load before / after (N) |',
            '|---|---:|---:|---|---|']
        for case in cases:
            ee=[e for e in partial if e['case_id']==case]
            if not ee:continue
            lines.append(f"| {case} | {len(ee)} | {sum(e['online_verified'] for e in ee)} | {min(e['actual_retract_mm'] for e in ee):.4f}–{max(e['actual_retract_mm'] for e in ee):.4f} | {np.mean([e['before_mean_normal_load_n'] for e in ee]):.3f} / {np.mean([e['after_mean_normal_load_n'] for e in ee]):.3f} |")
        if v and not v['verified_unloadings']:
            lines+=['', 'The tested 5 mm / 5 s recovery budget did not deliver sustained 0.5 mm actual retreat. No verified-arm insertion retry was authorized. Fewer repeated stalls in that arm therefore reflect termination after failed unloading, not evidence of successful recovery. This is a bounded negative result; it does not establish that unloading is impossible with other permitted commands or durations. No budget expansion was performed in this experiment.']
