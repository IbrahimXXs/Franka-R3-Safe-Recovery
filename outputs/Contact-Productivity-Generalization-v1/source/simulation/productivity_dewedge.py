"""Frozen-detector de-wedging pilot, with unchanged nominal and axial baselines."""
...
import time
import torch
from isaaclab.utils.math import quat_slerp, quat_apply, quat_mul, quat_conjugate
...
from research.productivity_dewedge import Dewedger, DewedgeDesign, neutral_quaternion, POLICIES
from productivity_unloading import execute_verified as execute_axial


def execute_dewedge(bench, case, frozen, unloading, dewedge, path):
    policy="productivity"
    ...
    unloader=None;retract_command=0.;relax_fraction=0.
    peg_anchor=peg_anchor_q=neutral_p=neutral_q=grasp_p=grasp_q=None
    retry_peg_p=retry_peg_q=retry_grasp_p=retry_grasp_q=None
    desired_peg_p=desired_peg_q=None
    ...
    def hand_from_peg(pp,pq,gp,gq):
        hq=quat_mul(pq,quat_conjugate(gq))
        return pp-quat_apply(hq,gp),hq
    def capture_grasp():
        e=bench.env; hp=e.fingertip_midpoint_pos;hq=e.fingertip_midpoint_quat
        return quat_apply(quat_conjugate(hq),e.held_pos-hp),quat_mul(quat_conjugate(hq),e.held_quat)
    def target(depth):
        if retry_peg_p is not None:
            pp=retry_peg_p.clone();pp[:,2]=bench.env.fixed_pos_obs_frame[:,2]-depth/1000.
            return hand_from_peg(pp,retry_peg_q,retry_grasp_p,retry_grasp_q)
    ...
    def fields(row, step, hp, hq):
        ...
        row.update(relax_fraction=relax_fraction,retry_pose_latched=retry_peg_p is not None)
        for i in range(3):row[f'command_peg_position_{i}']=desired_peg_p[0,i].item() if desired_peg_p is not None else None
        for i in range(4):row[f'command_peg_quaternion_{i}']=desired_peg_q[0,i].item() if desired_peg_q is not None else None
    ...
    try:
        for step in range(1,round((2+design.insertion_budget_s)*hz)+1):
            ...
            if t<=2.+1e-9:
                ...
                phase,relax_fraction,retract_command=unloader.target(t)
                desired_peg_p=peg_anchor+relax_fraction*(neutral_p-peg_anchor)
                desired_peg_p=desired_peg_p.clone();desired_peg_p[:,2]+=retract_command/1000.
                desired_peg_q=quat_slerp(peg_anchor_q[0],neutral_q[0].clone(),relax_fraction).unsqueeze(0)
                hp,hq=hand_from_peg(desired_peg_p,desired_peg_q,grasp_p,grasp_q)
                ...
                if mode=='intervene':
                    ...
                    retry_peg_p=bench.env.held_pos.clone();retry_peg_q=bench.env.held_quat.clone()
                    retry_grasp_p,retry_grasp_q=capture_grasp()
                    events[-1].update(retry_peg_position=retry_peg_p[0].tolist(),retry_peg_quaternion=retry_peg_q[0].tolist(),
                        retry_grasp_position=retry_grasp_p[0].tolist(),retry_grasp_quaternion=retry_grasp_q[0].tolist())
            ...
            if mode=='intervene':
                ...
                event.update(max_commanded_retract_mm=unloader.command_mm,max_actual_retract_mm=unloader.max_actual_mm,
                    ...
                    verification_time_s=unloader.verification_time,retry_ready_time_s=unloader.ready_time,hold_rebounds=unloader.rebounds,
                    max_relax_fraction=unloader.fraction)
            ...
            if step%round(design.check_s*hz)==0:
                ...
                if fired and not unsafe and success_dwell<round(p.success_dwell_s*hz) and interventions<design.max_interventions:
                    ...
                    unloader=Dewedger(t,anchor_depth,design.stop_s,unloading,dewedge);retract_command=0.;relax_fraction=0.
                    peg_anchor=bench.env.held_pos.clone();peg_anchor_q=bench.env.held_quat.clone()
                    grasp_p,grasp_q=capture_grasp()
                    neutral_p=peg_anchor.clone();neutral_p[:,:2]=bench.env.fixed_pos_obs_frame[:,:2]
                    neutral_q=torch.as_tensor(neutral_quaternion(peg_anchor_q[0].cpu().numpy(),bench.peg_q[0].cpu().numpy()),
                        device=peg_anchor_q.device,dtype=peg_anchor_q.dtype).unsqueeze(0)
                    events.append(dict(intervention=interventions,trigger_time_s=t,trigger_depth_mm=anchor_depth,
                        ...
                        trigger_peg_position=peg_anchor[0].tolist(),trigger_peg_quaternion=peg_anchor_q[0].tolist(),
                        neutral_peg_position=neutral_p[0].tolist(),neutral_peg_quaternion=neutral_q[0].tolist(),
                        captured_grasp_position=grasp_p[0].tolist(),captured_grasp_quaternion=grasp_q[0].tolist(),
                        nominal_peg_quaternion=bench.peg_q[0].tolist(),trigger_nominal_misalignment=command.copy(),
    ...
    metrics=dict(policy='dewedge',insertion_success=reason=='insertion_success',stalled=first_stall is not None,
        ...
        intervention_time_s=sum(r['phase'] in ('stop','relax','unload_retract','unload_hold','post_verify_hold','rejoin') for r in rows)*dt,
...
def run(args,app):
    ...
    unchanged=('research/productivity_control.py','simulation/productivity_control.py','simulation/forge_backend.py',
               'simulation/contact.py','simulation/productivity_unloading.py','research/productivity_unloading.py','research/forge_protocol.py','research/phase2_protocol.py','research/phase2b.py','experiments/forge_phase2.json')
    for f in unchanged:
    ...
    unloading=UnloadingDesign(**prior['unloading_design']).validate()
    if asdict(unloading)!=asdict(UnloadingDesign()):raise ValueError('Previous recovery budget changed')
    dewedge=DewedgeDesign(**{k:plan[k] for k in asdict(DewedgeDesign())}).validate(unloading)
    ...
    manifest=dict(schema=plan['schema'],status='running',physics_hz=args.physics_hz,physics_rate_selection=rate,
        seed=p.seed,protocol=asdict(p),design=frozen['design'],unloading_design=asdict(unloading),dewedge_design=asdict(dewedge),cases=cases,
        schedule=schedule,runs=[],previous_experiment=plan['previous_experiment'],
        detector_unchanged=True,calibration_unchanged=True,baseline_implementation='unmodified productivity_control.execute(nominal) and productivity_unloading.execute_verified',
    ...
    sources=[Path(__file__),root/'simulation/launch_productivity_dewedge.py',root/'research/productivity_dewedge.py',
        root/'research/productivity_dewedge_report.py',root/'research/productivity_unloading_report.py',root/'research/productivity_control_report.py',
        *(root/f for f in unchanged),root/'simulation/forge_experiment.py',root/'productivity_dewedge.sh',
        root/'tests/test_productivity_dewedge.py',root/'docs/productivity_dewedge.md',args.plan.resolve(),args.calibration.resolve(),case_plan_path]
    ...
    try:
        ...
        for i,spec in enumerate(schedule):
            ...
            print(f'[dewedging] {i+1}/{len(schedule)} {rid}',flush=True)
            if spec['policy']=='dewedge':metrics=execute_dewedge(bench,c['case'],frozen,unloading,dewedge,rd/'trajectory.csv')
            elif spec['policy']=='axial':metrics=execute_axial(bench,c['case'],frozen,unloading,rd/'trajectory.csv')
            else:metrics=execute_previous(bench,c['case'],'nominal',frozen,rd/'trajectory.csv')
            ...
            print(f"[dewedging] {metrics['outcome']}; depth={metrics['final_depth_mm']:.3f} stall={metrics['stalled']} interventions={metrics['intervention_count']} verified={sum(e.get('unloading_status')=='verified' for e in events)} wrist={metrics['max_wrist_force_n']:.3f} N load={metrics['max_normal_load_n']:.3f} N",flush=True)
    ...
    from research.productivity_dewedge_report import analyze
    analyze(directory)
