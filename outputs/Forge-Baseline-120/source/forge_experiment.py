...
from research.forge_protocol import protocol,pilot_cases,retained,screened,within_budget,clear,match,recovery_summary,RADIAL_CLEARANCE_MM
...
def reference(bench,case,seed,path,checkpoints,stop_step=None):
    p=protocol();first=bench.prepare(seed)
...
def run(args,app):
    directory=args.output_dir.resolve();directory.mkdir(parents=True);p=protocol()
    ...
    manifest={'study':'Forge-Controlled-Phase2-v1','status':'running','physics_hz':args.physics_hz,
        ...
        'sources':{src.name:hashlib.sha256(src.read_bytes()).hexdigest() for src in
            (Path(__file__),Path(__file__).with_name('forge_backend.py'),Path(__file__).parents[1]/'research/forge_protocol.py')}}
    (directory/'source').mkdir()
    for name in manifest['sources']:
        src=Path(__file__).with_name(name) if name!='forge_protocol.py' else Path(__file__).parents[1]/'research'/name
