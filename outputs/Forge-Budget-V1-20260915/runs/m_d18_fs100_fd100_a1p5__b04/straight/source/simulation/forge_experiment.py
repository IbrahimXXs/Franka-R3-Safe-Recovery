...
from research.forge_protocol import load_protocol,pilot_cases,retained,screened,within_budget,clear,match,recovery_summary,RADIAL_CLEARANCE_MM,radial_clearance
from research.forge_collection import accepted_slots,collection_cases,collection_status,validate_resume,family_quotas
from research.phase2b import load_plan,command_misalignment
from research.forge_gap_study import SCHEMA as GAP_SCHEMA, load_plan as load_gap_plan, TiltTriggerState
from research.forge_gap_metrics import recovery_phase_metrics, insertion_event_metrics
...
def reference(bench,case,seed,path,checkpoints,stop_step=None):
    ...
    progress=-10.
    gap_state=TiltTriggerState() if 'tilt_amplitude_deg' in case else None
    command=gap_state.command(case,0.) if gap_state else command_misalignment(case,progress)
    def target(d):return bench.target(d,**dict(x_mm=command['offset_x_mm'],y_mm=command['offset_y_mm'],
                                              roll_deg=command['roll_deg'],pitch_deg=command['pitch_deg']))
    def command_fields():
        values=dict(command_offset_x_mm=command['offset_x_mm'],command_offset_y_mm=command['offset_y_mm'],
                    command_roll_deg=command['roll_deg'],command_pitch_deg=command['pitch_deg'],
                    command_tilt_deg=(command['roll_deg']**2+command['pitch_deg']**2)**.5,
                    ramp_progress_depth_mm=progress)
        if gap_state:values.update(gap_state.as_dict())
        return values
    above,aq=target(-10.)
    ...
    try:
        first.update(**command_fields(),reference_step=0);stream.add(first)
        for step in range(1,(stop_step or count)+1):
            t=step*bench.dt
            # Legacy paths use depth feedback; gap-study ramps use a latched depth event and time.
            progress=max(progress,stream.rows[-1]['depth_mm'])
            command=gap_state.command(case,t) if gap_state else command_misalignment(case,progress)
            if t<=2:
            ...
            if gap_state:
                gap_state.update(case,t,r['depth_mm'],in_insertion=phase=='insert',physics_step=step)
            r.update(**command_fields(),reference_step=step)
            stream.add(r)
        ...
        if gap_state and stop_step is None:
            return stream.rows,event_checkpoints(stream.rows,p,bench.dt,bench.gap_probe_events),aborted
        if case.get('ramp_onset_mm') is not None and stop_step is None and stream.rows[-1]['depth_mm']>0:
            r=stream.rows[-1]
            # Distinct terminal state after the insertion/hold, even if a fixed
            # checkpoint at a similar depth was reached earlier.
            cps['terminal']=dict(depth_mm=r['depth_mm'],checkpoint_kind='terminal',reached=True,
                state=r.copy(),step=r['reference_step'],prefix_numerically_valid=bool(prefix_valid and not aborted),
                prefix_max_force_n=maximum_f,prefix_max_torque_nm=maximum_t,
                prefix_max_normal_load_n=maximum_load,prefix_max_penetration_mm=maximum_pen,probes=[])
...
def characterize(rows,bench,p,aborted):
    ...
    metrics=insertion_metrics(active or rows,bench.dt,p,radial_clearance(p),aborted)
...
def report(directory,manifest):
    ...
    with (directory/'report.md').open('a') as f:
        f.write('\n## Controlled FORGE interpretation\n\nPanda and stock SDF assets/contact settings; fixed nuisance parameters, '
                ...
                '**Labels are provisional:** the overlap screen and replay check do not establish timestep convergence. '
                'Valid-reference counts do not guarantee known recovery labels at every checkpoint.\n\n![Force profiles](force_profiles.png)\n\n![Force versus depth](force_vs_depth.png)\n')
...
def run(args,app):
    ...
    with (directory/'.run.lock').open('a') as lock:
        ...
        except BlockingIOError:raise RuntimeError('Another process is writing this output directory') from None
        return _run(args,app)


def _run(args,app):
    wall_start=time.monotonic()
    rate_selection=resolve_physics_rate(args)
    directory=args.output_dir.resolve();p=load_protocol(args.config)
    case_plan=None
    if args.case_plan is not None:
        schema=json.loads(args.case_plan.read_text()).get('schema')
        case_plan=load_gap_plan(args.case_plan) if schema==GAP_SCHEMA else load_plan(args.case_plan)
    gap_study=case_plan is not None and case_plan['schema']==GAP_SCHEMA
    if gap_study:
        gaps={c['radial_clearance_mm'] for c in case_plan['cases']}
        if len(gaps)!=1:
            raise ValueError('A simulator process uses one bore geometry. Use simulation/run_gap_study.py for a multi-gap plan.')
        gap=gaps.pop()
        if args.radial_clearance_mm is not None and not abs(args.radial_clearance_mm-gap)<1e-9:
            raise ValueError('Requested radial clearance differs from the frozen case plan')
        args.radial_clearance_mm=gap
    if case_plan is not None:p=replace(p,target_valid=len(case_plan['cases'])).validate()
    if args.target_valid is not None:p=replace(p,target_valid=args.target_valid).validate()
    if args.checkpoints is None:args.checkpoints=[] if gap_study else list(p.checkpoints_mm) if args.mode=='collect' else []
    if gap_study and args.checkpoints:
        raise ValueError('Gap study uses event checkpoints; select --gap-probe-events instead of fixed depths')
    ...
    manifest={'study':'Forge-Controlled-Phase2-v1','status':'running','physics_hz':args.physics_hz,
        ...
        'physics_rate_selection':rate_selection,
        'stock_buffers':args.stock_buffers,
        ...
        'gap_probe_events':args.gap_probe_events if gap_study else None,
        'sources':{src.name:hashlib.sha256(src.read_bytes()).hexdigest() for src in
            (Path(__file__),Path(__file__).with_name('forge_backend.py'),Path(__file__).with_name('forge_calibration.py'),
             ...
             Path(__file__).parents[1]/'research/forge_collection.py',Path(__file__).parents[1]/'research/phase2b.py',
             Path(__file__).parents[1]/'research/forge_gap_study.py',Path(__file__).parents[1]/'research/forge_gap_metrics.py',
             Path(__file__).parents[1]/'research/forge_geometry.py',Path(__file__).parents[1]/'research/forge_events.py',
             Path(__file__).parents[1]/'research/future_stall.py')}}
    if args.resume:
        ...
        for name in manifest['sources']:
            candidate=Path(__file__).parents[1]/'research'/name
            src=candidate if candidate.is_file() else Path(__file__).with_name(name)
            (directory/'source'/name).write_bytes(src.read_bytes())
    if args.mode=='collect':
        cases=collection_cases(p,manifest['attempts'],case_plan)
        manifest['planned_family_quotas']=dict(Counter(c['family'] for c in case_plan['cases'])) if case_plan else family_quotas(p)
    ...
    try:
        if args.mode=='collect' and collection_status(p,manifest['attempts'])!='paused':
            manifest['status']=collection_status(p,manifest['attempts'])
            save(directory/'study.json',manifest)
            report(directory,manifest)
            print(f"Collection: {manifest['valid_trajectories']}/{p.target_valid}; {manifest['status']}",flush=True)
            return
        bench=Bench(args,app)
        if args.radial_clearance_mm is not None:
            p=replace(p,effective_radial_clearance_mm=bench.scene_info['conservative_radial_clearance_mm']).validate()
        bench.protocol=p
        bench.gap_probe_events=args.gap_probe_events
        manifest['physics']['radial_clearance_mm']=radial_clearance(p)
        manifest['effective_protocol']=asdict(p)
        geometry=dict(variant=bench.scene_info.get('geometry_variant'),
                      socket_mesh_sha256=bench.scene_info.get('socket_mesh_sha256'),
                      effective_radial_clearance_mm=radial_clearance(p))
        if args.resume and manifest.get('geometry') is not None and manifest['geometry']!=geometry:
            raise ValueError('Geometry changed since the recorded study; cannot resume')
        ...
        new_attempts=0
        for case in cases:
            attempt_wall_start=time.monotonic()
            folder=directory/case['trajectory_id'];version=0
            while folder.exists():
                version+=1;folder=directory/f"{case['trajectory_id']}_resume{version}"
            folder.mkdir()
            attempt={**case,'folder':folder.name,'status':'running','checkpoints':[]}
            if gap_study:attempt['effective_radial_clearance_mm']=radial_clearance(p)
            ...
            if gap_study:attempt['metrics'].update(insertion_event_metrics(rows,case))
            ...
            if gap_study:attempt['final_retreat'].update(recovery_phase_metrics(rr,bench.dt),**recovery_termination(reason),termination_phase=rr[-1]['phase'])
            for cp in cps:
                cp.setdefault('checkpoint_id',cp['checkpoint_kind'] if cp.get('checkpoint_kind') else f'd{cp["depth_mm"]:g}')
                if cp['reached'] and cp['prefix_numerically_valid'] and cp.get('probe_requested',True):
                    for policy in ('straight','realign'):
                        tag=f'{cp["checkpoint_kind"]}_{policy}' if cp.get('checkpoint_kind') else f'd{cp["depth_mm"]:g}_{policy}'
                        ...
                        if matched and prefix_valid:
                            ...
                            if gap_study:probe.update(recovery_phase_metrics(recovery,bench.dt),**recovery_termination(why),termination_phase=recovery[-1]['phase'])
                ...
                if not cp.get('probe_requested',True):label,reason=None,'event_recorded_without_recovery_probe'
                cp.update(Y_R_tested=label,label_reason=reason)
            attempt['status']='complete';attempt['elapsed_wall_seconds']=time.monotonic()-attempt_wall_start
            save(folder/'trajectory.json',attempt);save(directory/'study.json',manifest)
            ...
            if args.mode=='collect':
                ...
                save(directory/'study.json',manifest)
                write_phase2_report(directory,manifest)
                print(f"Collection: {manifest['valid_trajectories']}/{p.target_valid} valid references",flush=True)
                if args.max_new_attempts is not None and new_attempts>=args.max_new_attempts:break
        manifest['status']=collection_status(p,manifest['attempts']) if args.mode=='collect' else 'pilot_complete'
        save(directory/'study.json',manifest);report(directory,manifest)
        ...
        manifest['status']='paused' if args.mode=='collect' and isinstance(error,KeyboardInterrupt) else 'error'
        manifest['error']=str(error)
        for attempt in manifest['attempts']:
            if attempt['status']=='running':attempt['status']='interrupted'
        raise
    finally:
        manifest['elapsed_wall_seconds']=previous_wall+time.monotonic()-wall_start
        save(directory/'study.json',manifest)
