...
Eligible-parent checkpoint labels: 2 safe witnesses, 0 tested-policy failures, 2 unknown. 0 checkpoint records belong to invalid or incomplete parents.
...
2 checkpoints lack two measured whole-recovery costs and are absent from the paired scatter; all remain in `comparison.csv`. Green: safe witness; red: both tested policies fail; grey: unknown/ineligible pair. No safe-only filter is applied.
