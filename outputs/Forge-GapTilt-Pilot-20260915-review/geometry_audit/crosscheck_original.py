...
Requirements: pxr (Usd/UsdGeom) plus the repository's pure forge_geometry helper.
...
def main():
    ...
    parser.add_argument('--hole-usd', type=Path, required=True)
    parser.add_argument('--peg-usd', type=Path, required=True)
    ...
    from research.forge_geometry import resize_socket_points, mesh_sha256
    ...
    selections = {'max_reported_overlap': min(range(len(rows)),key=lambda i:float(rows[i]['min_separation_mm'])),
                  'max_wrist_force': max(range(len(rows)),key=lambda i:float(rows[i]['wrist_force_n']))}
    output = {'profile':str(args.profile.resolve()),'radial_clearance_mm':args.radial_clearance_mm,
              'hole_mesh_sha256':mesh_sha256(hole_points,hole.GetFaceVertexCountsAttr().Get(),hole.GetFaceVertexIndicesAttr().Get()),
