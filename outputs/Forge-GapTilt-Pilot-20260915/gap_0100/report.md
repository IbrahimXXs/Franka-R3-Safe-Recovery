...
Status: **target_not_met**. 4 / 7 numerically valid trajectory slots filled.
...
Eligible-parent checkpoint labels: 3 safe witnesses, 0 tested-policy failures, 13 unknown. 60 checkpoint records belong to invalid or incomplete parents.
...
76 checkpoints lack two measured whole-recovery costs and are absent from the paired scatter; all remain in `comparison.csv`. Green: safe witness; red: both tested policies fail; grey: unknown/ineligible pair. No safe-only filter is applied.
