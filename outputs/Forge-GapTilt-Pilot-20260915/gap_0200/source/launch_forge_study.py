...
parser.add_argument('--case-plan', type=Path, help='Explicit Phase 2B or gap/tilt plan; requires collect mode')
parser.add_argument('--radial-clearance-mm', type=float,
                    help='Nominal one-sided bore clearance; fixed for this process, modifies the collision mesh')
parser.add_argument('--gap-probe-events', nargs='+', choices=('pre_tilt','ramp_complete','first_stall','terminal'),
                    default=['first_stall','terminal'], help='Gap-study event checkpoints to replay')
...
if args.radial_clearance_mm is not None and not 0.05 <= args.radial_clearance_mm <= .507:
    parser.error('--radial-clearance-mm must be in [0.05, 0.507] mm')
