...
import torch
from pxr import Usd,UsdPhysics,UsdGeom,Gf,Vt
...
from research.forge_geometry import (BASELINE_RADIAL_CLEARANCE_MM, BORE_RADIUS_M,
    PEG_RADIUS_M, audit_socket_points, mesh_sha256, resize_socket_points,
    validate_radial_clearance)


def prepare_socket_geometry(cfg,args):
    """Author point overrides before ForgeEnv spawns/cooks any physics assets."""
    clearance=getattr(args,'radial_clearance_mm',None)
    source=str(cfg.task.fixed_asset.spawn.usd_path)
    info={'source_usd':source,'geometry_modified':False,
          'clearance_definition':'radial_vertex_clearance_mm',
          'requested_radial_clearance_mm':clearance}
    if clearance is None:return info
    clearance=validate_radial_clearance(clearance)
    # The explicit baseline reuses the original asset without any point edits,
    # preserving existing cooked collision geometry and exact float values.
    if math.isclose(clearance,BASELINE_RADIAL_CLEARANCE_MM,abs_tol=1e-12):return info
    ...
    if not root.GetReferences().AddReference(source):
        raise RuntimeError(f'Could not reference socket asset: {source}')
    meshes=[p for p in stage.Traverse()
            if p.IsA(UsdGeom.Mesh) and p.HasAPI(UsdPhysics.CollisionAPI)]
    if len(meshes)!=1:raise RuntimeError(f'Expected one socket collider, found {len(meshes)}')
    prim=meshes[0];mesh=UsdGeom.Mesh(prim)
    points=mesh.GetPointsAttr().Get()
    counts=mesh.GetFaceVertexCountsAttr().Get();indices=mesh.GetFaceVertexIndicesAttr().Get()
    source_hash=mesh_sha256(points,counts,indices)
    edited=resize_socket_points(points,clearance)
    mesh.GetPointsAttr().Set(Vt.Vec3fArray([Gf.Vec3f(*p) for p in edited]))
    # The upstream USD embeds triangle-mesh and convex-hull cooked data. Those
    # buffers describe the original 9 mm hole and must not survive a point edit.
    blocked=[]
    for attr in prim.GetAttributes():
        if attr.GetName().startswith('physxCookedData:'):
            attr.Block();blocked.append(attr.GetName())
    readback=mesh.GetPointsAttr().Get()
    expected_radius=PEG_RADIUS_M+clearance/1000
    audit=audit_socket_points(readback,expected_bore_radius_m=expected_radius)
    geometry_dir=Path(args.output_dir).resolve()/'geometry'
    geometry_dir.mkdir(parents=True,exist_ok=True)
    destination=geometry_dir/f'socket_radial_{clearance:.6f}mm.usda'
    layer_text=stage.GetRootLayer().ExportToString()
    if destination.exists() and destination.read_text()!=layer_text:
        raise RuntimeError(f'Existing generated geometry differs: {destination}')
    if not destination.exists():destination.write_text(layer_text)
    cfg.task.fixed_asset.spawn.usd_path=str(destination)
    cfg.task.fixed_asset_cfg.usd_path=str(destination)
    cfg.task.fixed_asset_cfg.diameter=2*expected_radius
    info.update({'geometry_modified':True,'generated_usd':str(destination),
                 'generated_usd_sha256':hashlib.sha256(destination.read_bytes()).hexdigest(),
                 'source_mesh_sha256':source_hash,
                 'generated_mesh_sha256':mesh_sha256(readback,counts,indices),
                 'blocked_cooked_data_attributes':blocked,'pre_spawn_audit':audit})
    return info


def resolve_physics_rate(args):
    native=ForgeTaskPegInsertCfg()
    native_hz=round(1./native.sim.dt)
    if not math.isclose(native_hz*native.sim.dt,1.,rel_tol=1e-9):
        raise ValueError('FORGE timestep must correspond to an integer physics rate')
    source='upstream_default' if args.physics_hz is None else 'explicit_override'
    if args.physics_hz is None:args.physics_hz=native_hz
    print(f'FORGE physics: {args.physics_hz} Hz ({source}); installed scene default: {native_hz} Hz',flush=True)
    return {'source':source,'upstream_physics_hz':native_hz,'upstream_action_hz':native_hz/native.decimation}
...
class Bench:
    def __init__(self,args,app):
        ...
        self.geometry_info=prepare_socket_geometry(self.cfg,args)
        ...
        self.scene_info={'colliders':[],'joints':[],
                         'geometry_variant':self.geometry_info,
        ...
        held=[];fixed=[];socket_meshes=[];peg_radii=[]
        for prim in self.sim.stage.Traverse():
            ...
            if prim.HasAPI(UsdPhysics.CollisionAPI):
                ...
                if '/FixedAsset/' in path and prim.IsA(UsdGeom.Mesh):
                    ...
                    socket_meshes.append(UsdGeom.Mesh(prim))
                if '/HeldAsset/' in path and prim.IsA(UsdGeom.Mesh):
                    transform=UsdGeom.XformCache().GetLocalToWorldTransform(prim)
                    scales=[sum(transform[i][j]**2 for j in range(3))**.5 for i in range(3)]
                    if any(abs(s-1.)>1e-6 for s in scales):
                        raise RuntimeError('Peg geometry is scaled relative to the audited metre dimensions')
                    points=UsdGeom.Mesh(prim).GetPointsAttr().Get()
                    peg_radii.append(max(math.hypot(p[0],p[1]) for p in points))
        ...
        if len(socket_meshes)!=1 or len(peg_radii)!=1:
            raise RuntimeError(f'Expected one peg/socket mesh collider: {len(peg_radii)}, {len(socket_meshes)}')
        if abs(peg_radii[0]-PEG_RADIUS_M)>1e-6:
            raise RuntimeError('Peg geometry no longer matches the audited 7.986 mm diameter')
        mesh=socket_meshes[0];points=mesh.GetPointsAttr().Get()
        clearance=getattr(args,'radial_clearance_mm',None)
        expected_radius=BORE_RADIUS_M if clearance is None else PEG_RADIUS_M+clearance/1000
        self.scene_info.update(audit_socket_points(points,peg_radii[0],expected_radius))
        runtime_hash=mesh_sha256(points,mesh.GetFaceVertexCountsAttr().Get(),mesh.GetFaceVertexIndicesAttr().Get())
        self.scene_info['socket_mesh_sha256']=runtime_hash
        if self.geometry_info['geometry_modified'] and runtime_hash!=self.geometry_info['generated_mesh_sha256']:
            raise RuntimeError('Spawned socket mesh differs from generated geometry')
        self.geometry_info['runtime_mesh_sha256']=runtime_hash
    ...
    def observe(self,phase,command_depth):
        ...
        # PhysX advances with a float32 timestep; Lab's data timestamp adds the
        # Python float config value. Their small relative rounding difference
        # accumulates (about 1 microsecond after 19 seconds at 120 Hz).
        # Per-step continuity is independently checked in tick().
        if not math.isclose(row['physics_time_s'],row['time_s'],rel_tol=2**-23,abs_tol=1e-7):
