...
Status: **complete**. 7 / 7 numerically valid trajectory slots filled.
...
Eligible-parent checkpoint labels: 7 safe witnesses, 0 tested-policy failures, 21 unknown. 0 checkpoint records belong to invalid or incomplete parents.
...
24 checkpoints lack two measured whole-recovery costs and are absent from the paired scatter; all remain in `comparison.csv`. Green: safe witness; red: both tested policies fail; grey: unknown/ineligible pair. No safe-only filter is applied.

![Gap-study recovery comparison](gap_recovery_comparison.png)

### Terminal-state direct withdrawal by factors

Each cell is an observed terminal straight-withdrawal wrist peak, restricted to the retreat phase. Cells do not interpolate across configurations. S = safe complete recovery; F = valid tested failure; U = unknown/ineligible; ? = checkpoint exists but no retreat cost; dash = no recorded terminal checkpoint. Failed/censored values are partial-motion costs. Aligned controls remain in the tables and are excluded from the angle grid. If a case has retries, the latest recorded attempt is shown; all attempts remain in the tables.
