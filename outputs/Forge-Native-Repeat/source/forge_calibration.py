...
def run_calibration(bench,directory,seed):
    ...
    try:
        ...
        passed=bool(summary) and len(summary)==len(cases) and all(s['passed'] for s in summary)
        (directory/'calibration.json').write_text(json.dumps({'passed':passed,'cases':summary},indent=2)+'\n')
        lines=['# Wrist known-load calibration','',f'All eight load cases passed: **{passed}**.','',
               'Loads are applied at the measured peg COM in world axes, outside socket contact. '
               'Each case lasts three seconds; statistics use its final second. Static vector bias '
               'checks sign, axes, scale and the moment shift. Instantaneous residuals also include '
               'distal-link and grasp dynamics, and are bounded separately. This does not certify '
               'contact-force convergence across timesteps.','',
               'Limits: static force bias < 0.05 N, static torque bias < 0.002 Nm; mean instantaneous '
               'residual norm < 0.2 N and < 0.01 Nm; socket contact < 0.001 N.','',
               '| Load case | Force bias (N) | Torque bias (Nm) | Mean force residual norm (N) | Mean torque residual norm (Nm) | Passed |',
               '|---|---:|---:|---:|---:|---|']
        lines += [f'| {s["case"]} | {s["force_bias_n"]:.6f} | {s["torque_bias_nm"]:.6f} | '
                  f'{s["mean_force_residual_n"]:.6f} | {s["mean_torque_residual_nm"]:.6f} | {s["passed"]} |' for s in summary]
