...
def analyze(studies,output):
    ...
    for directory in studies:
        ...
        settings=fingerprint;attempts.extend(m['attempts'])
    ...
    for a in attempts:
        ...
        for i,c in enumerate(observed):
            ...
            if earlier:
                prev=earlier[-1]
                transitions.append(dict(trajectory_id=a['trajectory_id'],folder=a['folder'],path_group_id=a['path_group_id'],
    ...
    result=dict(sources=sources,transitions=transitions,recommendations=notes,
