...
ANGLE6_STUDY_ID = 'Forge-budget-angle6-v1'
ANGLE6_COMMAND_TILTS_DEG = (1.5, 2., 3., 4., 5., 6.)
...
def make_angle6_plan():
    """Independent user-requested command-angle extension after the budget pilot.

    The 1.5 degree candidate is repeated as an anchor in the new study. Larger
    command targets may terminate before attainment under the unchanged budget,
    numerical and grasp guards. Only the parameter allowlist is extended in the
    mechanics plan; its control equations and the physical model are unchanged.
    """
    original = make_plan()
    anchor = next(condition for condition in original['conditions'] if condition['role'] == 'candidate')
    conditions = []
    for angle in ANGLE6_COMMAND_TILTS_DEG:
        case = make_case(18., (1., 1.), angle)
        conditions.append(dict(anchor, condition_id=f'{case["case_id"]}__b04', case=case,
                               role='candidate' if angle == 1.5 else 'angle_extension'))
    source_sha = hashlib.sha256(json.dumps(original, sort_keys=True, allow_nan=False,
                                           separators=(',', ':')).encode()).hexdigest()
    return dict(original, conditions=conditions, study_id=ANGLE6_STUDY_ID,
                selection_provenance=dict(
                    selection_type='post_pilot_user_requested_angle_extension',
                    source_study='outputs/Forge-Budget-V1-20260915',
                    source_plan='experiments/forge_budget_v1.json',
                    source_plan_canonical_sha256=source_sha,
                    source_plan_canonical_encoding='sorted_keys_compact_json_utf8',
                    maximum_command_tilt_deg=6.,
                    anchor_condition_id=anchor['condition_id'],
                    anchor_definition='rerun of the completed 1.5 degree 4 N candidate in independent policy processes',
                    angle_definition='command target; actual tilt can remain below target when a guard terminates the reference',
                    parameter_validation_change='mechanics make_case allowlist adds 3, 4, 5 and 6 degrees; physical model and state/control equations unchanged',
                    independent_random_replications=False))
...
def load_plan(path):
    """Load either exact canonical matrix, including types, order and provenance."""
    ...
    try:
        ...
        equal = any(encoded == serialize(plan) for plan in (make_plan(), make_angle6_plan()))
    except (TypeError, ValueError):
        equal = False
    if not equal:
        raise ValueError('Budget plan differs from the exact Forge-budget-v1 canonical plans')
    return value
