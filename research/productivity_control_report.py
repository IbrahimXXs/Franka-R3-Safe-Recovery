...
def analyze(directory):
    ...
    for kind in ('depth','productivity','force'):
        ...
        for case,ax in zip(cases,axes[:,0]):
            for r in runs:
                ...
                if kind=='productivity':log=[x for x in log if x['check_performed']]  # None breaks undefined histories
                ...
                if kind=='depth' and log:
                    ax.scatter(log[-1]['time_s']-2,log[-1]['depth_mm'],color=color,s=35,zorder=5,
                        marker='x' if r['outcome'] not in ('insertion_success','time_budget_exhausted') else 'o' if r['insertion_success'] else 's')
    ...
    retraction_summary=[]
    for policy in ('force','productivity'):
        ee=[e for e in event_rows if e['policy']==policy]
        completed=[e for e in ee if 'actual_retraction_mm' in e]
        actual=[e['actual_retraction_mm'] for e in completed]
        first=[e for e in ee if e['intervention']==1]
        retraction_summary.append(dict(policy=policy,started=len(ee),completed_retractions=len(completed),
            incomplete_retractions=len(ee)-len(completed),
            median_actual_retraction_mm=float(np.median(actual)) if actual else None,
            min_actual_retraction_mm=min(actual) if actual else None,max_actual_retraction_mm=max(actual) if actual else None,
            first_triggers=len(first),first_triggers_before_own_stall=sum(not e['previous_stall'] for e in first),
            first_triggers_before_nominal_stall=sum(e['lead_to_nominal_stall_s'] is not None and e['lead_to_nominal_stall_s']>0 for e in first),
            strict_matched_first_triggers=sum(e['prefix_endpoint_matched'] for e in first),
            hidden_load_first_triggers=sum(e['normal_load_n']>=10 and e['wrist_force_n']<=5 for e in first)))
    table(directory/'retraction_summary.csv',retraction_summary)
    fig,axes=plt.subplots(1,2,figsize=(12,4))
    for j,policy in enumerate(('force','productivity')):
        ee=[e for e in event_rows if e['policy']==policy and 'actual_retraction_mm' in e]
        values=[e['actual_retraction_mm'] for e in ee]
        axes[0].scatter(j+np.linspace(-.12,.12,len(values)),values,color=colors[policy],label=policy)
        axes[1].scatter([e['wrist_force_n'] for e in ee],[e['normal_load_n'] for e in ee],color=colors[policy],label=policy)
    axes[0].axhline(m['design']['retract_mm'],color='black',ls=':',label='Commanded retract')
    axes[0].axhline(0,color='gray',lw=.5)
    axes[0].set(xticks=[0,1],xticklabels=['force','productivity'],ylabel='Net actual peg retreat since trigger (mm)',title='Completed retractions: every event')
    axes[1].set(xlabel='Wrist force at trigger (N)',ylabel='Privileged normal load at trigger (N)',title='Contact load can be hidden from wrist force')
    for ax in axes:ax.grid(alpha=.2);ax.legend(fontsize=8)
    fig.tight_layout();fig.savefig(directory/'retraction_response.png',dpi=150);plt.close(fig)
    ...
    lines+=['','## Observed behavior','',
        ...
        '|---|---:|---:|---:|---|']
    for case in cases:
        grouped={policy:[r for r in runs if r['case_id']==case and r['policy']==policy] for policy in POLICIES}
        fractions=[f"{sum(r['insertion_success'] for r in grouped[policy])}/{len(grouped[policy])}" for policy in POLICIES]
        depths=[f"{np.mean([r['final_depth_mm'] for r in grouped[policy]]):.3f}" if grouped[policy] else 'n/a' for policy in POLICIES]
        lines.append('| '+case+' | '+' | '.join(fractions)+' | '+' / '.join(depths)+' |')
    for r in retraction_summary:
        lines+=['',f"**{r['policy']}**: {r['started']} interventions started; {r['completed_retractions']} completed the retract phase and {r['incomplete_retractions']} were interrupted. Median net actual retreat was {r['median_actual_retraction_mm']:.4f} mm (range {r['min_actual_retraction_mm']:.4f}–{r['max_actual_retraction_mm']:.4f} mm), versus the 1 mm command. {r['first_triggers_before_own_stall']}/{r['first_triggers']} first interventions preceded that run's first confirmed stall. This timing does not imply the trigger preceded the 0.5 s stall lookback interval."]
    hidden=[e for e in event_rows if e['policy']=='productivity' and e['intervention']==1 and e['normal_load_n']>=10 and e['wrist_force_n']<1]
    if hidden:
        e=min(hidden,key=lambda e:e['wrist_force_n'])
        lines+=['',f"A hidden-load example is {e['case_id']} repeat {e['repeat']}: productivity triggered at eta={e['eta_raw']:.3f}, wrist force {e['wrist_force_n']:.3f} N and privileged contact load {e['normal_load_n']:.3f} N. Normal load was logged for analysis and did not enter the decision."]
    if len(a)==3 and a['productivity']['successes']<=a['nominal']['successes'] and a['productivity']['successes']<=a['force']['successes']:
        lines+=['', '**Conclusion for this fixed pilot:** productivity intervention did not improve insertion success. The progress signal can flag difficult contact at modest wrist force, but a commanded retract does not ensure an achieved retract, and retrying the same misalignment can reload the same contact. These results do not validate this fixed retract/retry policy as a stall-reducing controller. The next bounded experiment should first verify sufficient actual unloading/retraction, then test a revised retry action; no such tuning or expanded collection is part of this pilot.']
    lines+=['', 'Final depth and maximum-load comparisons include safety-terminated episodes with shorter exposure; a lower peak or shorter elapsed time on an aborted run is not a safety/performance gain. Depth plot end markers are circles for success, squares for timeout and crosses for a safety stop. Productivity plots leave undefined history intervals blank. All raw logs remain unchanged by report generation.']
    lines+=['','## Frozen design and calibration','',
        ...
        '![Productivity](productivity_comparison.png)', '', '![Wrist force](force_comparison.png)', '', '![Actual retreat](retraction_response.png)']
