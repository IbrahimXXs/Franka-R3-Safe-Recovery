...
from forge_experiment import Stream, guarded, save
from forge_mechanics_backend import MechanicsBench
from forge_mechanics_recovery import recover, MOTION
...
def recovery(bench, rows, policy, folder, basename, prefix_valid=True):
    with contact_recording(bench, folder/f'{basename}_contacts.jsonl.gz', initial=rows[-1]):
        samples, reason, motion = recover(bench, rows[-1], policy, bench.protocol, folder/f'{basename}.csv')
    ...
    metrics.update(mechanics_recovery_metrics(samples, bench.dt), reason=reason,
                   **recovery_termination(reason), termination_phase=samples[-1]['phase'], recovery_motion=motion)
    if reason == 'numerical_guard':
        metrics.update(numerically_valid=False, label_eligible=False, safe_recovery=None)
    return metrics
